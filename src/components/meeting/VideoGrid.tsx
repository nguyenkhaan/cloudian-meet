                    ...
                    <div className="flex flex-col gap-2">
                        {cameraTracks.map((track) => (
                            <ParticipantTile
                                ...
                                trackRef={track}
                                className={`${tileClass} aspect-video w-full shrink-0 [&_.lk-participant-media-video]:object-cover`}
                            />
                        ))}
                    </div>
                </aside>
        ...
        <div className={`mx-auto grid h-full w-full content-center gap-1.5 p-2 sm:gap-2 sm:p-3 ${layout}`}>
            {cameraTracks.map((track, index) => (
                <ParticipantTile 
                    key={`${track.participant.identity}-${track.source}`}
                    trackRef={track}
                    className={`${tileClass} aspect-video [&_.lk-participant-media-video]:object-cover ${index === 0 ? "" : "hidden xl:block"}`}
                /> 
            ))}
        </div>
    )
